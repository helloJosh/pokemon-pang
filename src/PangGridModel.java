...
public class PangGridModel {
	...
	private int pickachuCount = 0;
	private int raichuCombo = 0;
	private boolean upgrade = true;
	
	// 피카추가 라이츄로 진화하는 함수 
	public boolean pickaToRai() {
		if(pickachuCount >= 15){
			if(upgrade) {
				Sound.play("Evol");
				upgrade = false;
			...
			for(int r=0; r<PangUtility.NUMBEROFMONS; r++) {
				for(int c=0; c<PangUtility.NUMBEROFMONS; c++) {
						if(gridData[r][c] == Pokemon.values()[0])
							gridData[r][c] = Pokemon.values()[7];
	...
	private boolean checkRow(int[][] checkMatrix){
		...
		for(int r=0; r<PangUtility.GRIDSIZE; r++){
			for(int c=0; c<PangUtility.GRIDSIZE-2; c++){
				if(gridData[r][c].ordinal()<(PangUtility.NUMBEROFMONS+1)){
					int count = 1;
					for(int i=c+1; i<PangUtility.GRIDSIZE; i++){
						if(gridData[r][c]==gridData[r][i]) { ++count;
							if(gridData[r][c] ==Pokemon.values()[7])
								++raichuCombo;
						}
						else break;
					}
		...
		*/
		...
		for(int c=0; c<PangUtility.GRIDSIZE; c++){
			...
			for(int r=0; r<PangUtility.GRIDSIZE; r++){
				if(gridData[r][c]==Pokemon.POKEBALL){
					if(pickaToRai())
						gridData[r][c] = Pokemon.values()[randomGen.nextInt(PangUtility.NUMBEROFMONS+1)];
					else
						gridData[r][c] = Pokemon.values()[randomGen.nextInt(PangUtility.NUMBEROFMONS)];
				}
				else break;
